...
BEGIN_MESSAGE_MAP(CFreecellStarter2020Dlg, CDialogEx)
	...
	ON_WM_LBUTTONUP()
	ON_WM_ERASEBKGND()
	ON_COMMAND(ID_MENU_QUIT, &CFreecellStarter2020Dlg::OnMenuQuit)
	ON_COMMAND(ID_QUIT_RESTART, &CFreecellStarter2020Dlg::OnQuitRestart)
	ON_COMMAND(ID_MENU_CHEATMODE, &CFreecellStarter2020Dlg::OnMenuCheatmode)
	ON_COMMAND(ID_MENU_CHEATMODEOFF, &CFreecellStarter2020Dlg::OnMenuCheatmodeoff)
END_MESSAGE_MAP()
...
void CFreecellStarter2020Dlg::OnPaint()
{
	...
	else
	{
		...
		CBrush lPinkBrush(RGB(255, 192, 203));
		dc.SelectObject(lPinkBrush);
...
void CFreecellStarter2020Dlg::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	else if (mFirstClick != -1 && mCells[mFirstClick] == mCells[selected]) {
	...
	else // second of two click
	{
		...
		if (CheckWin()) {
			MessageBox(L"Way to go, you've won", MB_OK);
		}
	}
...
void CFreecellStarter2020Dlg::MyDrawing(CDC& dc)
{
	...
	COLORREF gcBackgroundColor = (RGB(0, 0, 0)); // your choice!
...
void CFreecellStarter2020Dlg::OnMenuQuit()
{
...
void CFreecellStarter2020Dlg::OnQuitRestart()
{
	for (int j = 0; j < 16; j++) {
		mCells[j]->EmptyCell();
	}
...
void CFreecellStarter2020Dlg::OnMenuCheatmode()
{
	for (int i = 8; i < 16; i++) {
		mCells[i]->ToggleCheat(true);
	}
}


void CFreecellStarter2020Dlg::OnMenuCheatmodeoff()
{
	for (int i = 8; i < 16; i++) {
		mCells[i]->ToggleCheat(false);
	}
}
bool CFreecellStarter2020Dlg::CheckWin() {
	for (int i = 4; i < 8; i++)
	{
		if (mCells[i]->CanRemoveCard()) {
			if ((mCells[i]->GetTopCard()) / 4 != 12) {
				return false;
			}
		}
		else {
